...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        PushMatrix(model);
            ...
            model = model * Matrix_Translate(0, 0.30f, 0.0f); // ### CABECA 
            model = model * Matrix_Scale(0.4f, 0.25f, 1.2f);
            PushMatrix(model);
        ...
        PushMatrix(model); // Guardamos matriz model atual na pilha
            ...
            PushMatrix(model); // Guardamos matriz model atual na pilha
                ...
                //PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                    ...
                    model = model * Matrix_Translate(0.0f, -0.65f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com a translação do mao direita
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        ...
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                        DrawCube(render_as_black_uniform); // #### MAO DIREITA // Desenhamos a mao direita
                    PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
            PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
        PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
            ...
            PushMatrix(model); // Guardamos matriz model atual na pilha
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    ...
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        ...
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                        DrawCube(render_as_black_uniform); // #### ANTEBRAÇO DIREITO // Desenhamos o antebraço esquerdo
                    PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                //PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente

                    model = model * Matrix_Translate(0.0f, -0.65f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com a translação do mao esquerda
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        model = model * Matrix_Scale(0.2f, 0.1f, 0.2f); // Atualizamos matriz model (multiplicação à direita) com um escalamento da mao esquerda
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                        DrawCube(render_as_black_uniform); // #### MAO DIREITA // Desenhamos a mao esquerda
                    PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
            PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
        PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
// pernas
        ...
        PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(-0.2f, -1.04f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com uma translação para a perna direita 
            PushMatrix(model); // Guardamos matriz model atual na pilha
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Scale(0.25f, 0.6f, 0.3f); // Atualizamos matriz model (multiplicação à direita) com um escalamento 
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                    DrawCube(render_as_black_uniform); // #### BRAÇO DIREITO // Desenhamos o braço direito
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Translate(0.0f, -0.65f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com a translação do antebraço esquerdo
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        model = model * Matrix_Scale(0.25f, 0.6f, 0.2f); // Atualizamos matriz model (multiplicação à direita) com um escalamento do antebraço esquerdo
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                        DrawCube(render_as_black_uniform); // #### ANTEBRAÇO DIREITO // Desenhamos o antebraço esquerdo
                    PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                //PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente

                    model = model * Matrix_Translate(0.0f, -0.65f, 0.1f); // Atualizamos matriz model (multiplicação à direita) com a translação do mao esquerda
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        model = model * Matrix_Scale(0.25f, 0.1f, 0.4f); // Atualizamos matriz model (multiplicação à direita) com um escalamento da mao esquerda
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                        DrawCube(render_as_black_uniform); // #### MAO DIREITA // Desenhamos a mao esquerda
                    PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
